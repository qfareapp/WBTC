...
const { collectEligibleTripOffersForConductor } = require("../controllers/conductorTrip.controller");
...
const buildNotificationText = (offers) => {
  ...
  return {
    title: "New Conductor Trip Offer",
    body: content || "Your driver has accepted a trip. Review the conductor offer.",
  };
};
...
const logTicketResults = ({ conductorId, tripIds, tokenList, tickets }) => {
  tickets.forEach((ticket, index) => {
    const token = maskToken(tokenList[index]);
    const base = `[conductor-offer-notifier] ticket conductor=${conductorId} tripIds=${tripIds.join(",")} token=${token}`;
    if (ticket?.status === "ok") {
...
const scheduleReceiptLookup = ({ conductorId, tripIds, tickets }) => {
  ...
  setTimeout(async () => {
    try {
      ...
      receiptIds.forEach((receiptId) => {
        ...
        const base = `[conductor-offer-notifier] receipt conductor=${conductorId} tripIds=${tripIds.join(",")} receiptId=${receiptId}`;
      ...
      console.error(
        `[conductor-offer-notifier] receipt_lookup_failed conductor=${conductorId} tripIds=${tripIds.join(",")} ${error.message}`
      );
    }
  }, RECEIPT_LOOKUP_DELAY_MS);
};

const pruneInvalidTokens = async (conductor, invalidTokens) => {
  if (!invalidTokens.length) return;
  const invalidSet = new Set(invalidTokens);
  conductor.pushTokens = (conductor.pushTokens || []).filter(
    (item) => !invalidSet.has(String(item?.token || "").trim())
  );
  await conductor.save();
};

const notifyConductorForOffers = async (conductor, date) => {
  const targets = selectPreferredPushTargets(normalizePushTargets(conductor.pushTokens));
  if (!targets.length) return;

  const result = await collectEligibleTripOffersForConductor({ conductorId: conductor._id, date, debug: false });
  ...
  const existingOffers = await ConductorOffer.find({
  ...
  if (expoTargets.length) {
    ...
    logTicketResults({
      conductorId: String(conductor._id),
  ...
  await pruneInvalidTokens(conductor, invalidTokens);
  if (!delivered) return;

  await Promise.all(
    newOffers.map((offer) =>
      ConductorOffer.findOneAndUpdate(
        { tripInstanceId: offer.tripInstanceId, conductorId: conductor._id },
...
const syncConductorOfferNotifications = async () => {
  ...
  try {
    ...
    for (const conductor of conductors) {
      try {
        await notifyConductorForOffers(conductor, date);
      } catch (error) {
        console.error(`[conductor-offer-notifier] conductor=${conductor._id} ${error.message}`);
      }
    }
  } catch (error) {
    console.error(`[conductor-offer-notifier] ${error.message}`);
  } finally {
    syncInProgress = false;
  }
};

const startConductorOfferNotifier = () => {
  if (process.env.CONDUCTOR_OFFER_NOTIFICATIONS_ENABLED === "false") return;
  if (intervalHandle) return;

  const intervalMs = Math.max(
    Number(process.env.CONDUCTOR_OFFER_NOTIFICATION_INTERVAL_MS || DEFAULT_INTERVAL_MS),
    15000
  );

  intervalHandle = setInterval(() => {
    syncConductorOfferNotifications().catch((error) => {
      console.error(`[conductor-offer-notifier] ${error.message}`);
    });
  }, intervalMs);
